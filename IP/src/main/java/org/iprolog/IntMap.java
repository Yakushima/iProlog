 ...
 */
  ...
  final static IntStack intersect(final IntMap[] maps, final IntMap[] vmaps) {
    ...
    // Main.println ("maps[0].size()=" + maps[0].m_size);
    ...
    // Main.println ("vmaps[0].size()=" + vmaps[0].m_size);
    intersect0(vmaps[0], maps, vmaps, r);
    // Main.println ("  intersection: " + r.toString());
  ...
  final private void rehash(final int newCapacity) {
    // Prog.println("           *** rehash: newCapacity=" + newCapacity);
